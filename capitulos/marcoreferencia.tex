...
El UART controla los puertos y dispositivos serie para comincaciones. La tarjeta de desarllo Altera DE2-115 tiene implementado un puerto RS-232, el cual no se utilizo y se opto por utilizar y personalizar los pines de comunicacion utilizando el ''Expansion Header (J15)'' para fines practicos mostrado en la Figura \ref{fig:imagen3}. Se desarrollaron los bloques para general el módulo que se encarga de implementar la UART personalizado en Verilog. para los pines respectivos en el Hexpansion Header.
...
Para fines demostrativos del cifrado y que la información se esta enviando correctamente se implemento la comunicación estandard hacia unos ordenadores, hay recordar que que no podemos implementar directamente la comunicacion de la tarjeta de desarollo Altera DE2-115 con nuestro ordenador siendo que uno de los objetivos fue el modificar el protocolo UART para crear una capa de seguridad extra a nivel Hardware y si esta se conecta con el ordenador este solo tendra disponible la comunicacion bajo el estandard UART original y la comunicacion entre estas dos plataformas PC y FPGA no sera exitosa. Para esto se implemento otra FPGA similar Altera DE2-115 para que en esta se implemente la misma aquitectura y le comunicacion entre FPGA's sea exitosa e ideal siendo que en una aplicacion real no requeriremos de los ordenadores para monitorear la informacion. 
